  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl text-[#1C3A54]" style={{ fontWeight: 700 }}>Kalender Kegiatan</h1>
      ...
      {/* custom events */}
      <div>
        <h3 className="text-[#1C3A54] mb-3 flex items-center gap-2" style={{ fontWeight: 600 }}><Sparkles className="size-5 text-[#A8802F]" /> Kegiatan RT 02</h3>
        {customEvents.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-[#1C3A54]/20 bg-white p-10 text-center text-muted-foreground">Belum ada kegiatan. Klik "Tambah Kegiatan".</div>
        ) : (
              ...
              <div key={e.id} className="bg-white rounded-2xl border border-black/5 p-5">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-[#1C3A54] line-clamp-1" style={{ fontWeight: 600 }}>{e.title}</p>
                ...
                <div className="mt-4 flex gap-2">
                  <button onClick={() => openEdit(e)} className="flex-1 flex items-center justify-center gap-1 py-2 rounded-lg bg-[#1C3A54]/10 text-[#1C3A54] text-sm hover:bg-[#1C3A54] hover:text-white transition"><Pencil className="size-4" /> Edit</button>
                  <button onClick={() => setDel(e)} className="grid place-items-center size-9 rounded-lg bg-red-50 text-[#9C2B2F] hover:bg-[#9C2B2F] hover:text-white transition"><Trash2 className="size-4" /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div>
        <h3 className="text-[#1C3A54] mb-1 flex items-center gap-2" style={{ fontWeight: 600 }}>
          <Flag className="size-5 text-[#9C2B2F]" /> Hari Besar Nasional & Islam (Otomatis)
        </h3>
        ...
        <div className="bg-white rounded-2xl border border-black/5 divide-y divide-black/5">
          {autoEvents.map((e) => (
            <div key={e.id} className="flex items-center gap-3 p-3">
              <span className={`grid place-items-center size-9 rounded-lg shrink-0 ${e.type === "islamic" ? "bg-green-100 text-green-600" : "bg-[#9C2B2F]/10 text-[#9C2B2F]"}`}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Edit Kegiatan" : "Tambah Kegiatan"}>
        <form onSubmit={save} className="space-y-4">
          ...
          <label className="flex items-center justify-between p-3 rounded-xl bg-[#F6F2EA]">
            <span className="text-sm">Tampilkan hitung mundur di kalender</span>
            <input type="checkbox" checked={form.showCountdown} onChange={(e) => setForm({ ...form, showCountdown: e.target.checked })} className="accent-[#1C3A54] size-5" />
          </label>
