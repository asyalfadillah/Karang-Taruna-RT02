    ...
    return (
      <div className="min-h-screen grid place-items-center pt-20">
        <div className="text-center">
          <p className="text-muted-foreground">Album tidak ditemukan.</p>
          <Link to="/" className="mt-4 inline-block text-[#0F4C81] hover:underline">← Kembali ke Beranda</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="pt-16">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-7xl mx-auto px-4 md:px-8 pb-8 w-full text-white">
            <Link to="/#dokumentasi" className="inline-flex items-center gap-2 text-white/80 hover:text-white mb-4 text-sm">
              <ArrowLeft className="size-4" /> Kembali ke Dokumentasi
            </Link>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-sm text-white/85">
              ...
              <span className="flex items-center gap-1"><ImageIcon className="size-4" /> {albumPhotos.length} Foto</span>
              <span className="flex items-center gap-1"><Film className="size-4" /> {albumVideos.length} Video</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-10">
        ...
        {album.driveLink && (
          ...
          >
            <FolderOpen className="size-5" /> Lihat Semua Dokumentasi <ExternalLink className="size-4" />
          </a>
        )}
          ...
          {(["foto", "video"] as const).map((t) => (
            <button
              ...
              className={`px-5 py-3 capitalize transition border-b-2 -mb-px ${
                tab === t ? "border-[#D32F2F] text-[#D32F2F]" : "border-transparent text-muted-foreground hover:text-foreground"
              }`}
              style={{ fontWeight: 600 }}
            >
              {t} ({t === "foto" ? albumPhotos.length : albumVideos.length})
            </button>
          ))}
        </div>
        ...
        {tab === "video" &&
          (albumVideos.length === 0 ? (
            <p className="text-muted-foreground py-10 text-center">Belum ada video pada album ini.</p>
          ) : (
