...
export function VideoGallery() {
  ...
  return (
    <section id="video" className="py-24 bg-[#0b3660] text-white">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {videos.map((v, i) => (
            <Reveal key={v.id} delay={(i % 3) * 0.1}>
              <div className="group rounded-2xl overflow-hidden bg-white/5 border border-white/10 hover:border-[#D4AF37]/50 transition">
                <button onClick={() => openVideo(v)} className="relative block w-full aspect-video overflow-hidden">
                  ...
                  <span className="absolute inset-0 bg-black/30 grid place-items-center">
                    <span className="grid place-items-center size-16 rounded-full bg-[#D32F2F] text-white group-hover:scale-110 transition shadow-xl">
                      <Play className="size-7 ml-1" fill="currentColor" />
                    </span>
                  </span>
                  <span className="absolute top-3 left-3 px-2 py-1 rounded bg-white/90 text-[#0F4C81] text-xs">{VIDEO_SOURCE_LABEL[v.source]}</span>
